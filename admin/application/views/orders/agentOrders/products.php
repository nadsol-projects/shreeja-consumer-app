...
$adata = $this->db->get_where("fdm_va_auths",["id"=>$this->session->userdata("admin_id")])->row();
...
<!--						<h5 class="m-b-20">Accordion</h5>-->
			...
			$search_value = "and product_name like '%$search%'";
				...
				$cid = 0;
					...
					foreach($categories as $cat){
						...
						$products = $this->db->query("select * from tbl_products where status='Active' and deleted=0 and product_category='$cat->id' and assigned_to='agents' and (JSON_CONTAINS(location,'[\"".$adata->city."\"]')) > 0 $search_value GROUP BY product_id order by product_name asc")->result();
	...
	<div id="collapse<?php echo $cid ?>" class="collapse show" aria-labelledby="heading<?php echo $cid ?>" data-parent="#accordion">
		...
		<form action="<?php echo base_url("orders/agent-order/") ?><?php echo ($oid != "") ? 'editOrder' : 'insertOrder' ?>" method="post" enctype="multipart/form-data">							
...
<!--		<div class="card m-b-5">-->
		...
		<form action="<?php echo base_url("orders/agent-orders/editOrder") ?>" method="post" enctype="multipart/form-data">							
...
<!--						<input type="hidden" name="product_id" id="product_id">-->
			...
			$aid = isset($u->agent_id) ? $u->agent_id : '';
							?>
								...
								<div class="col-sm-12 col-md-5">
									...
									<div class="form-group">
										...
										<select class="form-control" name="<? echo ($aid != '') ? '' :'aid' ?>" id="agts" data-live-search="true" style="width: 100%;height: 50px;" <? echo ($aid != '') ? 'disabled' :'' ?> required>
											...
											if(count($semp) > 0){	
												...
												$agents = json_decode($this->admin->get_admin("assigned_agents",$agts));
													...
													foreach($agents->agents as $ag){
														...
														$role_name = $this->db->get_where("fdm_va_roles",["id"=>$ag->role])->row()->role_name;

														$aa = $this->db->get_where("fdm_va_auths",array("status"=>"Active","deleted"=>0,"id"=>$ag))->row();

														$achk = ($aid == $aa->id) ? 'selected' : '';

														echo '<option value="'.$aa->id.'" '.$achk.'>'.$aa->name." - ".$aa->agent_id." (".$role_name.")".'</option>';

													}
												}
											}else{
												
												$agents = json_decode($this->admin->get_admin("assigned_agents"));
												...
												$agents = $this->db->where_in("role",[2,3,4,5,13])->get_where("fdm_va_auths",array("status"=>"Active","deleted"=>0))->result();

												foreach($agents as $a){
													
													$role_name = $this->db->get_where("fdm_va_roles",["id"=>$a->role])->row()->role_name;

													$achk = ($aid == $a->id) ? 'selected' : '';

													echo '<option value="'.$a->id.'" '.$achk.'>'.$a->name." - ".$a->agent_id." (".$role_name.")".'</option>';
										...
										<? echo ($aid != '') ? '<input type="hidden" name="aid" value="'.$aid.'">' :'' ?>
				...
				<?php if(isset($u->transaction_document)){ ?>		
					<div class="container" align="right">
						...
						<a target="_blank" href="<? echo base_url('orders/depositOrders/transactiondocuments/').$u->order_id ?>" class="btn btn-info waves-effect waves-light">View Transaction Document</a>
...
$(document).ready(function(){
    ...
    $('.deliveryDate').bootstrapMaterialDatePicker({ weekStart: 0, time: false,minDate: new Date(),dateFormat : 'mm/dd/yy'});    
