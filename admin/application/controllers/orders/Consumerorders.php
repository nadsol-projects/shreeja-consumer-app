...
public function activesubscribers()
{
	...
	$this->load->view('orders/admin/activesubscribers');
}

public function activesubscribersOrders(){
	
	$this->db->where("sdate <=", date("Y-m-d"));
	$this->db->where("edate >=", date("Y-m-d"));
	$data = $this->db->get_where("orders",array("payment_status"=>"Success","order_type"=>"subscribe"))->result();
